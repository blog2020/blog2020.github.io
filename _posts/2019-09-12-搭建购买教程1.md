...
然后点击确定，会提示你保存一个东西，你就点击保存，然后会弹框让你输入密码，就把之前保存的那一串数字填进去，就可以进入你的服务器了。
